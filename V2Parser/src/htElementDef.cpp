...
bool HtmlElementDef::canInclude(HtmlDTD::HtmlTags aTag)
{
#if !defined(BE_RELAX)
    if ((tag == HtmlDTD::tPcdata) || (tag == HtmlDTD::tCdata)) return false;
#else
    if (tag == HtmlDTD::tPcdata) return true;
    if (tag == HtmlDTD::tCdata) return false;
#endif
...
bool HtmlAttrDef::isNamed(char *aName)
{
	if (name != NULL)
