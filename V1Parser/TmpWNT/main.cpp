...
main(int argc, char **argv)
{
	...
	for (i= 0; i < nbrFilesIn; i++) {
		...
		case SPIT_JAVA:
#if defined(_WIN32)
		    if ((outputStream= new ofstream("/tmp/htmlOut.java", ios::binary)) != (ofstream *)-1) {
#else
		    if ((outputStream= new ofstream("/tmp/htmlOut.java")) != (ofstream *)-1) {
#endif
			javaSpitter= new JavaRenderer(outputStream);
...
#else
    while ((c = getopt(nbrArgs, args, "dhtjrs")) != EOF) {
	switch((char)c) {
	    case 'h':
		ShowUtil();
		exit(0);
